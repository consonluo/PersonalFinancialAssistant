...
class AccountDetailPage extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: accountAsync.whenOrNull(data: (a) => Text(a?.name ?? '')) ??
            const Text('账户详情'),
      ),
      body: Center(
          ...
          child: holdingsAsync.when(
            data: (holdings) {
              ...
              for (final h in holdings) {
                ...
                totalMv += mv;
                totalCost += h.quantity * h.costPrice * rate;
                todayChg +=
                    market != null ? mv * market.changePercent / 100 : 0.0;
              }
              final totalPnl = totalMv - totalCost;

              final account = accountAsync.valueOrNull;
              final financingAmount = account?.financingAmount ?? 0;
              final financingCurrency =
                  (account?.financingCurrency ?? 'CNY').isEmpty
                      ? 'CNY'
                      : (account?.financingCurrency ?? 'CNY');
              final financingCny = financingAmount * getRate(financingCurrency);
              final netAccountValue = totalMv - financingCny;
                ...
                children: [
                  ...
                  if (holdings.isNotEmpty) ...[
                    ...
                    ...holdings.map((h) {
                      ...
                      final pnlPct = h.costPrice != 0
                          ? (price - h.costPrice) / h.costPrice * 100
                          : 0.0;
                      ...
                      return Dismissible(
                        ...
                        child: Card(
                          ...
                          child: InkWell(
                            onTap: () => context.push(
                                '/holding-form?id=${h.id}&accountId=$accountId'),
                                  ...
                                  Row(
                                    children: [
                                      ...
                                      _DetailChip(
                                        label: '今日',
                                        value:
                                            '${todayChgPct >= 0 ? "+" : ""}${todayChgPct.toStringAsFixed(2)}%',
                                        color: todayChgPct >= 0
                                            ? AppColors.gain
                                            : AppColors.loss,
                                      ),
                  ...
                  if (plans.isNotEmpty) ...[
                    ...
                    ...plans.map((p) => Card(
                          ...
                          child: ListTile(
                            ...
                            title: Text(p.assetName,
                                style: const TextStyle(
                                    fontSize: 14, fontWeight: FontWeight.w500)),
                            subtitle: Text('${p.frequency} · ${p.assetCode}',
                                style: const TextStyle(
                                    fontSize: 12,
                  ...
                  if (memberLiabilities.isNotEmpty) ...[
                    _SectionHeader(
                        title: '关联负债', count: memberLiabilities.length),
                    const SizedBox(height: 8),
                          ...
                          child: ListTile(
                            ...
                            title: Text(l.name,
                                style: const TextStyle(
                                    fontSize: 14, fontWeight: FontWeight.w500)),
                            subtitle: Text(
                                '${l.type} · 利率 ${l.interestRate.toStringAsFixed(2)}%',
                                style: const TextStyle(
                                    fontSize: 12,
                                    color: AppColors.textSecondary)),
                            trailing: Column(
